...
las <- list.files("./03.density_4_LiDAR/", pattern = "*.las", full.names =  T)
...
las.names <- list.files("./03.density_4_LiDAR/", pattern = "*.las", full.names =  F) #set location where all plot .las were added
...
for(i in 1:length(las.names)) tryCatch({
  ...
  data.40m <- readLAS(file.path(paste0("./03.density_4_LiDAR/", las.names[i])))
  ...
  data.40m <- readLAS(file.path(paste0("./03.density_4_LiDAR/", las.names[i])), filter = "-drop_z_below .5")
  ...
  normlas.file <- paste0("./03.density_4_LiDAR/", las.names[i])
...
FSD[FSD$plot_area_ground <= 1300, ] 

FSD <- FSD[FSD$plot_area_ground > 1300, ] #dropped 6

#save the results
write.csv(FSD, file = "./04.Data_table/disturbance_structural_diversity_data_2022_1109_den4.csv")
